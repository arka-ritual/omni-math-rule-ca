#!/usr/bin/env python3
"""Unified API-based inference for SWE-bench Pro.
...
from inference.swe_dataset_loader import format_problem_for_inference
...
async def run_inference(args):
    ...
    if args.system_prompt:
        ...
        if args.prompt not in PROMPTS_SWE:
            raise ValueError(f"Unknown prompt preset '{args.prompt}'. Available: {list(PROMPTS_SWE.keys())}")
        prompt_text = PROMPTS_SWE[args.prompt]
    ...
    async def process(item: dict):
        nonlocal completed
        # Format problem for SWE-bench
        problem_text = format_problem_for_inference(item)
        user_msg = f"{user_prefix}\n\n{problem_text}" if user_prefix else problem_text
        async with sem:
        ...
        result = dict(item)
        result["model_patch"] = response or ""
...
def parse_args():
    parser = argparse.ArgumentParser(description="API-based inference for SWE-bench Pro")
    parser.add_argument("--provider", type=str, default="anthropic", help="Provider name (default: anthropic)")
    parser.add_argument("--model", type=str, required=True, help="Model name (e.g. claude-opus-4-6)")
    parser.add_argument("--data_file", type=str, default="swe_bench_pro_100.jsonl", help="Path to dataset JSONL")
