#!/usr/bin/env bash
# In-distribution (Omni-MATH eval slice) eval of the fixed-quant_m25 checkpoints
# + the original model across an arbitrary set of CA framings. Same structure as
# run_hle_ood.sh but on data/abstention_ft/eval.jsonl, writing into the standard
# abstention_ft namespace so the summary/plot scripts pick the cells up.
#
# Defaults to the two framings missing from the earlier sweep (quant_m1,
# quant_m0_25); override PROMPTS to add others.
#
# Resumable: skips any cell whose results already exist.
#
# Usage:
#   bash inference/run_checkpoint_framings.sh                          # gemma, quant_m1 + quant_m0_25
#   MODEL=qwen bash inference/run_checkpoint_framings.sh               # qwen3.5-9b
#   PROMPTS="quant_m1 quant_m0_25 quant_m5" bash inference/run_checkpoint_framings.sh
set -euo pipefail
...
PROMPTS="${PROMPTS:-quant_m1 quant_m0_25}"
END="${END:-100}"
MAX_MODEL_LEN="${MAX_MODEL_LEN:-16700}"
MAX_TOKENS="${MAX_TOKENS:-16384}"
INCLUDE_ORIGINAL="${INCLUDE_ORIGINAL:-1}"
...
echo "[done] checkpoint framing runs complete for MODEL=$MODEL (PROMPTS='$PROMPTS')."
